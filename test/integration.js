...
const xmlDevicesGroups = String(
  fs.readFileSync(path.join(__dirname, "./data/") + "test_api_response.xml")
);
//var xmlDevicesGroups = fs.readFileSync('./test.xml');

const xmlTemplate = String(
  fs.readFileSync(path.join(__dirname, "./data/") + "template_answer.xml")
);
const xmlTriggerlist = String(
  fs.readFileSync(path.join(__dirname, "./data/") + "getriggerlistinfos.xml")
);
const xmlTempStats = String(
  fs.readFileSync(
    path.join(__dirname, "./data/") + "devicestat_temp_answer.xml"
  )
);
const xmlPowerStats = String(
  fs.readFileSync(
    path.join(__dirname, "./data/") + "devicestat_power_answer.xml"
  )
);
const xmlColorDefaults = String(
  fs.readFileSync(path.join(__dirname, "./data/") + "color_defaults.xml")
);
const hkr_batt = fs.readFileSync(
  path.join(__dirname, "./data/") + "hkr_response.xml"
);
const guestWlan = fs.readFileSync(
  path.join(__dirname, "./data/") + "guest_wlan_form.xml"
);
...
describe("Test of Fritzdect-AHA-API", () => {
  ...
  it("function getdevicelistinfos", async () => {
    ...
    const devices = []
      .concat((devicesgroups.devicelist || {}).device || [])
      .map((device) => {
        // remove spaces in AINs
        device.identifier = device.identifier.replace(/\s/g, "");
        return device;
    ...
    const groups = []
      .concat((devicesgroups.devicelist || {}).group || [])
      .map((group) => {
        // remove spaces in AINs
        group.identifier = group.identifier.replace(/\s/g, "");
        return group;
  ...
  it("function getswitchpresent", async () => {
    ...
    const switchpresent = apiresponse["devicelist"]["device"]
      .filter(
        (device) =>
          device.hasOwnProperty("present") && device.identifier === ain
      )
  ...
  it("function getswitchpower", async () => {
    ...
    const switchpower = apiresponse["devicelist"]["device"]
      .filter(
        (device) =>
          device.hasOwnProperty("powermeter") && device.identifier === ain
      )
  ...
  it("function getswitchname", async () => {
    ...
    const switchname = apiresponse["devicelist"]["device"]
      .filter(
        (device) => device.hasOwnProperty("name") && device.identifier === ain
      )
  ...
  it("function gethkrtsoll", async () => {
    ...
    const tsoll = apiresponse["devicelist"]["device"]
      .filter(
        (device) => device.hasOwnProperty("hkr") && device.identifier === ain
      )
