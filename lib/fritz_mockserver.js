...
//hashing stuff
const challenge = (4294967295 + Math.floor(Math.random() * 4294967295))
  .toString(16)
  .slice(-8);
const challenge2 = (4294967295 + Math.floor(Math.random() * 4294967295))
  .toString(16)
  .slice(-8);
...
class FritzEmu {
  ...
  setupHttpServer(callback) {
    ...
    server = http.createServer((request, response) => {
      console.log(
        "HTTP-Server (Fritzbox Emulation): Request: " +
          request.method +
          " " +
          request.url
      );
      ...
      if (reqstring[0] == "/login_sid.lua") {
        if (version == 2) {
          //console.log(request);
          response = this.loginoutAnswerV2(
            response,
            sid,
            request.method,
            username,
          ...
          response = this.loginoutAnswerV1(
            response,
            sid,
            request.method,
            username,
        ...
        reqstring[0] == "/webservices/homeautoswitch.lua" &&
        sid === mocksid
      ) {
        ...
        response.writeHead(200, {
          "xmlDevicesGroups-Type": "application/json",
        });
        ...
        request.on("end", () => {
          ...
          if (
            ...
            response.writeHead(200, {
              "xmlDevicesGroups-Type": "application/xml",
            });
            response.write(
              JSON.stringify({
                data: {
        ...
        console.log(
          "\x1b[31m",
          "->  not supported call " + request.method + "  " + request.url
        );
    ...
    server.listen(this.emuport, () => {
      ...
      console.log(
        "\x1b[34m",
        "MOCK HTTP-Server (Fritzbox Emulation) listening on: http://localhost:" +
          this.emuport
      );
      console.log();
      console.log(
        "\x1b[34m",
        "for testing, setup in iobroker for second instance admin:password"
      );
      console.log(
        "\x1b[0m",
        "-------------------------------------------------------------------"
    ...
    switch (switchcmd) {
      ...
      case "getdeviceinfos":
        ...
        if (deviceinfos) {
          ...
          console.log(
            "\x1b[31m",
            " did not find the ain in devices/groups " + ain
          );
      ...
      case "applytemplate":
        const template = this.apiresponse["templatelist"]["template"].filter(
          (template) =>
            template.hasOwnProperty("identifier") && template.identifier === ain
        );
        if (template) {
          response.writeHead(200, {
            "xmlDevicesGroups-Type": "application/json",
      ...
      case "setswitchtoggle":
        const setswitchstate = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("switch") && device.identifier === ain
          )
          .map((device) => device.switch.state);
        const setgroupstate = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) =>
              group.hasOwnProperty("switch") && group.identifier === ain
          )
      ...
      case "gettemperature":
        const gettemp = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("temperature") && device.identifier === ain
          )
          .map((device) => device.temperature.celsius);
        const getgrouptemp = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) =>
              group.hasOwnProperty("temperature") && device.identifier === ain
          )
      ...
      case "getswitchpresent":
        ...
        const devicevalue = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) => device.hasOwnProperty(item) && device.identifier === ain
          )
          .map((device) => device[item]);
        const groupvalue = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) => group.hasOwnProperty(item) && group.identifier === ain
          )
      ...
      case "gethkrabsenk":
        ...
        const gethkrtemp = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("hkr") && device.identifier === ain
          )
          .map((device) => device.hkr[item3]);
        const getgrouphkrtemp = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) => group.hasOwnProperty("hkr") && group.identifier === ain
          )
      ...
      case "setsimpleonoff":
        ...
        const simplevalue = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("simpleonoff") && device.identifier === ain
          )
          .map((device) => device.simpleonoff.state);
        const groupsimplevalue = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) =>
              group.hasOwnProperty("simpleonoff") && group.identifier === ain
          )
      ...
      case "setlevel":
        ...
        const levelvalue = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("levelcontrol") && device.identifier === ain
          )
          .map((device) => device.levelcontrol.level);
        const grouplevel = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) =>
              group.hasOwnProperty("levelcontrol") && group.identifier === ain
          )
          .map((group) => group.levelcontrol.level);
        if (levelvalue.length > 0) {
          const pos = this.findAin("device", ain);
          this.apiresponse.devicelist.device[pos].levelcontrol.level = level;
          this.apiresponse.devicelist.device[pos].levelcontrol.levelpercentage =
            Math.floor((Number(level) / 255) * 100);
      ...
      case "setlevelpercentage":
        ...
        if (levelvalueperc.length > 0) {
          const pos = this.findAin("device", ain);
          this.apiresponse.devicelist.device[pos].levelcontrol.level =
            Math.floor((Number(level) / 100) * 255);
          this.apiresponse.devicelist.device[pos].levelcontrol.levelpercentage =
            level;
          response.statusCode = 200;
          response.write(String(level));
          ...
          const pos = this.findAin("group", ain);
          this.apiresponse.devicelist.group[pos].levelcontrol.level =
            Math.floor((Number(level) / 100) * 255);
          this.apiresponse.devicelist.device[pos].levelcontrol.levelpercentage =
            level;
          response.statusCode = 200;
      ...
      case "setcolor":
        ...
        const colorvalue = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("colorcontrol") && device.identifier === ain
          )
          .map((device) => device.colorcontrol[colorcmd]);
        const groupcolor = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) =>
              group.hasOwnProperty("colorcontrol") && group.identifier === ain
          )
          .map((group) => group.colorcontrol[colorcmd]);
        const newvalue = hue || saturation;
        if (colorvalue.length > 0) {
          const pos = this.findAin("device", ain);
          this.apiresponse.devicelist.device[pos].colorcontrol[colorcmd] =
            newvalue;
          response.statusCode = 200;
          ...
          const pos = this.findAin("group", ain);
          this.apiresponse.devicelist.group[pos].colorcontrol[colorcmd] =
            newvalue;
          response.statusCode = 200;
          //todo was kommt wirklich zurück?
          response.write(String(newvalue));
      ...
      case "setcolortemperature":
        ...
        if (settempvalue.length > 0) {
          const pos = this.findAin("device", ain);
          this.apiresponse.devicelist.device[pos].colorcontrol.temperature =
            temperature;
          response.statusCode = 200;
          //todo was kommt wirklich zurück?
          ...
          const pos = this.findAin("group", ain);
          this.apiresponse.devicelist.group[pos].colorcontrol.temperature =
            temperature;
          response.statusCode = 200;
          //todo was kommt wirklich zurück?
          response.write(String(temperature));
      ...
      case "sethkrboost":
        ...
        if (parseInt(endtimestamp) * 1000 > now.getTime() + 86400) {
          ...
          const pos = this.findAin("device", ain);
          this.apiresponse.devicelist.device[pos].hkr.boostactiveendtime =
            endtimestamp;
          response.writeHead(200, {
            "xmlDevicesGroups-Type": "application/json",
          });
          ...
          const pos = this.findAin("group", ain);
          this.apiresponse.devicelist.group[pos].hkr.boostactiveendtime =
            endtimestamp;
          response.writeHead(200, {
            "xmlDevicesGroups-Type": "application/json",
          });
      ...
      case "sethkrwindowopen":
        ...
        if (parseInt(endtimestamp) > now2.getTime() + 86400) {
          ...
          const pos = this.findAin("group", ain);
          this.apiresponse.devicelist.group[pos].hkr.windowopenactiveendtime =
            endtimestamp;
          response.writeHead(200, {
            "xmlDevicesGroups-Type": "application/json",
          });
      ...
      case "setblind":
        ...
        const blindvalue = this.apiresponse["devicelist"]["device"]
          .filter(
            (device) =>
              device.hasOwnProperty("blind") && device.identifier === ain
          )
          .map((device) => device.blind.mode);
        const groupblind = this.apiresponse["devicelist"]["group"]
          .filter(
            (group) => group.hasOwnProperty("blind") && group.identifier === ain
          )
      ...
      case "settriggeractive":
        ...
        const trigger = this.apiresponse["triggerlist"]["trigger"].filter(
          (trigger) =>
            trigger.hasOwnProperty("identifier") && trigger.identifier === ain
        );
        if (trigger.length > 0) {
          response.writeHead(200, {
            "xmlDevicesGroups-Type": "application/json",
  ...
  findAin(type, ain) {
    ...
    if (type === "device") {
      for (
        let i = 0;
        i < this.apiresponse["devicelist"]["device"].length;
        i++
      ) {
  ...
  xml2ApiResponse(xmlDevGroups, xmlTemplates, xmlTrigger) {
    ...
    let devices = []
      .concat((devices2json.devicelist || {}).device || [])
      .map((device) => {
    ...
    let groups = []
      .concat((devices2json.devicelist || {}).group || [])
      .map((group) => {
        // remove spaces in AINs
    ...
    let templates = []
      .concat((templates2json.templatelist || {}).template || [])
