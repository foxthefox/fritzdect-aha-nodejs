...
class Fritz {
  ...
  async setLevelPercentage(ain, level) {
    try {
      ain = ain.replace(/\s/g, "");
      const body = await this.executeCommand2(
        "setlevelpercentage&level=" + level,
        ain,
        1
      );
      return Promise.resolve(body);
  ...
  async get_sid(box_url, username, password, sidORrights) {
    ...
    try {
      ...
      if (state.pbkf2 === true) {
        ...
        challenge_response = this.calculate_pbkdf2_response(
          state.challenge,
          password
        );
        ...
        challenge_response = this.calculate_md5_response(
          state.challenge,
          password
        );
    ...
    if (box_url) {
      ...
      const options = {
        ...
        headers: {
          ...
          "User-Agent": "fritzdect_aha_nodejs/2.x",
        },
        ...
        const req = http.request(options, (res) => {
          ...
          res.on("end", () => {
            if (responseBody) {
              ...
              const challenge = responseBody.match(
                "<Challenge>(.*?)</Challenge>"
              )[1];
              const blocktime = Math.floor(
                responseBody.match("<BlockTime>(.*?)</BlockTime>")[1]
              );
  ...
  async send_response(box_url, username, challenge_response) {
    if (box_url && username && challenge_response) {
      ...
      const xFormBody = `${encodeURI("username")}=${encodeURI(
        username
      )}&${encodeURI("response")}=${encodeURI(challenge_response)}`;
      ...
      if (!box_url) console.log("Please provide the IP Address.");
        ...
        console.log("Please provide the user session token.");
    }
  }
  ...
  async executeCommand2(command, ain, loop) {
    ...
    // path includes the whole command string including SID
    try {
      const response = await this.fritzAHA_Request(
        reqpath,
        boxurl,
        this.options
      );
  ...
  async login_SID() {
    try {
      const sid = await this.get_sid(
        this.url.url,
        this.username,
        this.password,
        true
      ); //instead of using url.url better to use the boxurl
  ...
  async getPermissions() {
    ...
    try {
      const rights = await this.get_sid(
        this.url.url,
        this.username,
        this.password,
        false
